function countDown() {
  const diasCasamento = new Date('06 September 2024 16:30:00');
...
function sugerirMusica(event) {
  if (input.value.length > 0 || sugerir.value.length > 0) {
    const link = `https://api.whatsapp.com/send/?phone=351963531739&text=Olá,+gostaria+de+sugerir+a+música+para+a+festa :+${
      input.value
